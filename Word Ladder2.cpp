    ...
    int ladderLength(string start, string end, unordered_set<string> &dict) {
		...
		for (int i = 1; i<=n; i++)
		{
			...
			if(mark[j] == 0&&
			    dist[j] > dist[minj]+1)
			{
				pair<int, int> p_now = make_pair(minj,j);

				if(edge.find(p_now)!=edge.end())
					dist[j] = dist[minj]+1;
			}
		}
...
//	{"sand", "acne", 
	...
	Solution sol;
	int ans = sol.ladderLength(start, end, x);
